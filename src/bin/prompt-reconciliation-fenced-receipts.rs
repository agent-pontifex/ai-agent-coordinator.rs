...
mod tests {
    ...
    fn reacquisition_increments_fence_and_rejects_stale_writer() {
        ...
        let second = state.acquire("worker-b", 151, 50);
        assert!(second.is_ok());
        ...
        assert_eq!(
            state.record(&first, 152, 0, receipt("DEN-1")),
            Err(StateError::StaleFence)
        );
        assert_eq!(
            state.record(&second, 152, 0, receipt("DEN-1")),
            Ok(ReceiptOutcome::Recorded)
        );
    }
    ...
    fn expired_token_cannot_mutate_or_renew() {
        let mut state = FencedReceiptState::default();
        let token = state.acquire("worker-a", 100, 50);
        assert!(token.is_ok());
        let Ok(token) = token else {
            return;
        };
        assert_eq!(
            state.record(&token, 150, 0, receipt("DEN-1")),
            Err(StateError::LeaseExpired)
        );
        assert_eq!(state.renew(&token, 150, 50), Err(StateError::LeaseExpired));
    }

    #[test]
    fn receipt_compare_and_set_is_monotonic() {
        let mut state = FencedReceiptState::default();
        let token = state.acquire("worker-a", 100, 100);
        assert!(token.is_ok());
        let Ok(token) = token else {
            return;
        };
